...
class _MarketInfoPageState extends State<MarketInfoPage> {
  ...
  MarketProfileModel oldData;
  Future<MarketProfileModel> getData() async {
    if (oldData == null || readData) {
      var headers = await AuthService.getDefaultHeadersNoLocation();
      var adres = DataService.marketBaseaddress + 'Market/GetCustomerProfile';
      var res = await http.get(Uri.http(DataService.authority, adres),
          headers: headers);
      if (res.statusCode == 200) {
        var oldData = MarketProfileModel.fromJson(json.decode(res.body));
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
        future: getData(),
        builder: (context, AsyncSnapshot<MarketProfileModel> res) {
          if (res.connectionState == ConnectionState.done) {
            return buildScaffold(res.data);
          } else {
            return Scaffold(
              appBar: AppBarNew(
                title: Lang(context).marketInfo,
              ),
  ...
  Widget buildScaffold(MarketProfileModel res) {
    return Scaffold(
      appBar: AppBarNew(
  ...
  Future save() async {
    if (_formKey.currentState.validate()) {
      ...
      var adres =
          DataService.customerBaseaddress + 'Market/ChangeMarketProfile';
