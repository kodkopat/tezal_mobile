...
import 'package:tezal/models/Customer/MarketDetailsbyCustomerModel.dart';
...
class MarketDetailPage extends StatefulWidget {
  final String id;
  const MarketDetailPage({Key key, @required this.id}) : super(key: key);
...
class _MarketDetailPageState extends State<MarketDetailPage> {
  MarketDetailModel oldData;
  String id;

  _MarketDetailPageState(this.id);

  Future<MarketDetailModel> getMaerketDetail() async {
    var headers = await AuthService.getDefaultHeadersNoLocation();
    var adres = DataService.customerBaseaddress + 'Market/GetMarketDetail';
    var res = await http.get(Uri.http(DataService.authority, adres, {'Id': id}),
        headers: headers);

    if (res.statusCode == 200) {
      oldData = MarketDetailModel.fromJson(json.decode(res.body));
      return oldData;
    }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
        ...
        builder: (context, AsyncSnapshot<MarketDetailModel> res) {
  ...
  Widget buildScaffold(MarketDetailModel res, bool loading) {
    return Scaffold(
        appBar: AppBarNew(
          title: Lang(context).market,
          actions: [
